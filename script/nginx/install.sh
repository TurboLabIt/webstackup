...
NGINX_INSTALL_WSU_DIR=/usr/local/turbolab.it/webstackup/
if [ ! -f "${NGINX_INSTALL_WSU_DIR}setup.sh" ]; then
  curl -s https://raw.githubusercontent.com/TurboLabIt/webstackup/master/setup.sh?$(date +%s) | sudo bash
fi

source "${NGINX_INSTALL_WSU_DIR}script/base.sh"
...
ln -s "${WEBSTACKUP_CONFIG_DIR}nginx/05_global_default_vhost_disable.conf" /etc/nginx/conf.d/
...
ln -s "${WEBSTACKUP_CONFIG_DIR}nginx/02_global_http_level.conf" /etc/nginx/conf.d/
