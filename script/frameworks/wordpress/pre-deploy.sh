...
fxTitle "Enabling OPcache timestamp revalidation (WordPress-safe)..."
## The global 30-webstackup-opcache.ini sets opcache.validate_timestamps=0 (OPcache never
## re-checks files on disk). That's unsafe for WordPress, which rewrites its own PHP files on
## core/plugin/theme auto-updates without reloading php-fpm. Symlink the override into conf.d as
## 35-* so it loads after (and wins over) 30-webstackup-opcache.ini.
