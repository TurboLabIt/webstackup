...
fxTitle "Config path"
PHP_CONFIG_DIR=/etc/php/${PHP_VER}/
PHP_FPM_CONFIG_DIR=${PHP_CONFIG_DIR}fpm/conf.d/
PHP_CLI_CONFIG_DIR=${PHP_CONFIG_DIR}cli/conf.d/
echo "FPM: ${PHP_FPM_CONFIG_DIR}"
echo "CLI: ${PHP_CLI_CONFIG_DIR}"
...
ln -s  "${WEBSTACKUP_INSTALL_DIR}config/php/php-custom.ini" ${PHP_FPM_CONFIG_DIR}30-webstackup-custom.ini
ln -s  "${WEBSTACKUP_INSTALL_DIR}config/php/php-custom.ini" ${PHP_CLI_CONFIG_DIR}30-webstackup-custom.ini

fxTitle "Disabling execution function from FPM..."  
ln -s  "${WEBSTACKUP_INSTALL_DIR}config/php/no-exec.ini" ${PHP_FPM_CONFIG_DIR}30-webstackup-no-exec.ini

fxTitle "Setting timezone to Italy..."
ln -s  "${WEBSTACKUP_INSTALL_DIR}config/php/timezone-italy.ini" ${PHP_FPM_CONFIG_DIR}30-webstackup-timezone-italy.ini
ln -s  "${WEBSTACKUP_INSTALL_DIR}config/php/timezone-italy.ini" ${PHP_CLI_CONFIG_DIR}30-webstackup-timezone-italy.ini
fxInfo "Different timezone? rm -rf ${PHP_FPM_CONFIG_DIR}30-webstackup-timezone-italy.ini ${PHP_CLI_CONFIG_DIR}30-webstackup-timezone-italy.ini"

fxTitle "Removing all limits from CLI..."
ln -s  "${WEBSTACKUP_INSTALL_DIR}config/php/unlimited.ini" ${PHP_CLI_CONFIG_DIR}35-webstackup-unlimited.ini

fxTitle "Disable cgi.fix_pathinfo from FPM..."
ln -s  "${WEBSTACKUP_INSTALL_DIR}config/php/no-cgi.fix_pathinfo.ini" ${PHP_FPM_CONFIG_DIR}30-webstackup-no-cgi.fix_pathinfo.ini

fxTitle "Enable OPcache..."
ln -s  "${WEBSTACKUP_INSTALL_DIR}config/php/opcache.ini" ${PHP_FPM_CONFIG_DIR}30-webstackup-opcache.ini
...
if [ "$INSTALLED_RAM" -gt "6000" ]; then
  ...
  ln -s "${WEBSTACKUP_INSTALL_DIR}config/php/fpm-pool-32GB.conf" ${PHP_CONFIG_DIR}fpm/pool.d/webstackup-fpm-pool-32GB.conf

else

  echo "RAM: ${INSTALLED_RAM}: this is a small server! Using fpm-pool-1GB.conf"
  ln -s "${WEBSTACKUP_INSTALL_DIR}config/php/fpm-pool-1GB.conf" ${PHP_CONFIG_DIR}fpm/pool.d/webstackup-fpm-pool-1GB.conf
fi
