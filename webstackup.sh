...
if [ $INSTALL_PHP = 1 ]; then
	...
	printMessage "Activating custom config..."
	ln -s  "${WEBSTACKUP_INSTALL_DIR}config/php/php-custom.ini" /etc/php/${PHP_VER}/fpm/conf.d/30-custom-${SCRIPT_NAME}.ini
	ln -s  "${WEBSTACKUP_INSTALL_DIR}config/php/php-custom.ini" /etc/php/${PHP_VER}/cli/conf.d/30-custom-${SCRIPT_NAME}.ini
	
	printMessage "Set timezone to Italy..."
	ln -s  "${WEBSTACKUP_INSTALL_DIR}config/php/timezone-italy.ini" /etc/php/${PHP_VER}/fpm/conf.d/30-timezone-italy-${SCRIPT_NAME}.ini
	ln -s  "${WEBSTACKUP_INSTALL_DIR}config/php/timezone-italy.ini" /etc/php/${PHP_VER}/cli/conf.d/30-timezone-italy-${SCRIPT_NAME}.ini
...
if [ $INSTALL_MYSQL = 1 ]; then
	...
	if [ ! -e "${MYSQL_CREDENTIALS_FULLPATH}" ]; then
		...
		printTitle "Writing MySQL root credentials to ${MYSQL_CREDENTIALS_FULLPATH}..."
...
if [ $INSTALL_XDEBUG = 1 ]; then
	...
	printTitle "Activating custom xdebug config..."
...
if [ $INSTALL_POSTFIX = 1 ]; then
	...
	printTitle "Adding the postfix user to the opendkim group..."
	adduser postfix opendkim
	
	printTitle "Wiring together opendkim and postfix..."
